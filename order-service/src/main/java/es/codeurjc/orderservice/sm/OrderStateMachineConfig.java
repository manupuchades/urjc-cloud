...
public class OrderStateMachineConfig extends StateMachineConfigurerAdapter<OrderStatusEnum, OrderEventEnum>{
	...
	private final Action<OrderStatusEnum, OrderEventEnum> deliverOrderAction;
	private final Action<OrderStatusEnum, OrderEventEnum> deliverFailureAction;
    ...
    public void configure(StateMachineTransitionConfigurer<OrderStatusEnum, OrderEventEnum> transitions) throws Exception {
	   ...
	   .and().withExternal()
	   		.source(OrderStatusEnum.ALLOCATED).target(OrderStatusEnum.DELIVERY_PENDING)
			.event(OrderEventEnum.DELIVER_ORDER)
			.action(deliverOrderAction)
	   .and().withExternal()
			.source(OrderStatusEnum.DELIVERY_PENDING).target(OrderStatusEnum.DELIVERED)
			.event(OrderEventEnum.DELIVERY_SUCCESS)
	   .and().withExternal()
			.source(OrderStatusEnum.DELIVERY_PENDING).target(OrderStatusEnum.REJECTED)
			.event(OrderEventEnum.DELIVERY_FAILED)
			.action(deliverFailureAction)
	   .and().withExternal()
	        .source(OrderStatusEnum.DELIVERED).target(OrderStatusEnum.CREDIT_PENDING)
