...
public class OrderService {
    ...
    public CreateOrderResponse placeOrder(CreateOrderRequest createOrderRequest) {
        final OrderDetails orderDetails = OrderDetails.builder()
                .customerId(createOrderRequest.getCustomerId())
                .orderTotal(new Money(createOrderRequest.getOrderTotal()))
                .cityId(createOrderRequest.getCityId())
                .build();
    ...
    public GetOrderResponse getOrder(UUID orderId) {
        ...
        return GetOrderResponse.builder().orderId(order.getId())
                .orderState(order.getState())
                .rejectionReason(order.getRejectionReason())
                .deliveryId(order.getOrderDetails().getDeliveryId())
                .cityId(order.getOrderDetails().getCityId())
                .build();
    }
}
